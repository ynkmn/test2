...
def find_process_folders(directory):
    ...
    result = {
        ...
        "required_folders": {       # 必須フォルダの存在状態
            ...
            "process_c": {
                "exists": False,
                "required_file_patterns": ["def_*.dat", "inp.csv"],  # 変更: 複数のファイルパターン
                "found_files": {"def_*.dat": [], "inp.csv": []},     # 変更: パターン別ファイル情報
                "min_required_files": 100,                           # def_*.datファイルの最小必要数
                "files_requirement_met": False,
                "has_inp_csv": False                                # inp.csvの存在状態
            }
        },
        "status": {                 # 全体のステータス情報
    ...
    try:
        ...
        # process_c フォルダのチェック（変更された部分）
        if "process_c" in process_folders:
            result["required_folders"]["process_c"]["exists"] = True
            
            # def_*.dat ファイルをチェック
            def_pattern = os.path.join(directory, "process_c", "def_*.dat")
            def_files = glob.glob(def_pattern)
            result["required_folders"]["process_c"]["found_files"]["def_*.dat"] = [os.path.basename(f) for f in def_files]
            ...
            # inp.csv ファイルをチェック
            inp_csv_path = os.path.join(directory, "process_c", "inp.csv")
            inp_csv_exists = os.path.exists(inp_csv_path)
            result["required_folders"]["process_c"]["has_inp_csv"] = inp_csv_exists
            if inp_csv_exists:
                result["required_folders"]["process_c"]["found_files"]["inp.csv"] = ["inp.csv"]
            else:
                result["required_folders"]["process_c"]["found_files"]["inp.csv"] = []
            
            # def_*.datが最小数を満たし、かつinp.csvが存在する場合に要件を満たす
            min_required = result["required_folders"]["process_c"]["min_required_files"]
            result["required_folders"]["process_c"]["files_requirement_met"] = (
                len(def_files) >= min_required and inp_csv_exists
            )
...
def print_check_results(results):
    ...
    if results["required_folders"]["process_c"]["exists"]:
        # def_*.datファイルの状況
        def_files = results["required_folders"]["process_c"]["found_files"]["def_*.dat"]
        def_count = len(def_files)
        min_required = results["required_folders"]["process_c"]["min_required_files"]
        def_status = "✓ 必要数のファイルが存在します" if def_count >= min_required else f"✗ 必要なファイル数が不足しています ({def_count}/{min_required}必要)"
        print(f"    - def_*.dat: {def_status}")
        
        if def_count > 0:
            print(f"      - 検出されたファイル数: {def_count}")
            ...
            # 最初の3ファイルのサンプルを表示
            file_samples = def_files[:3]
            if file_samples:
                print(f"      - サンプルファイル: {', '.join(file_samples)}" + 
                      (" ..." if def_count > 3 else ""))
        
        # inp.csvファイルの状況
        inp_status = "✓ 存在します" if results["required_folders"]["process_c"]["has_inp_csv"] else "✗ 見つかりません"
        print(f"    - inp.csv: {inp_status}")
        
        # 全体のファイル要件
        files_req_met = results["required_folders"]["process_c"]["files_requirement_met"]
        overall_status = "✓ すべてのファイル要件を満たしています" if files_req_met else "✗ ファイル要件が満たされていません"
    ...
    if results["status"]["all_requirements_met"]:
        ...
        if not results["status"]["required_files_exist"]:
            ...
            # process_c のファイル問題を表示（変更された部分）
            if results["required_folders"]["process_c"]["exists"] and not results["required_folders"]["process_c"]["files_requirement_met"]:
                def_files = len(results["required_folders"]["process_c"]["found_files"]["def_*.dat"])
                needed = results["required_folders"]["process_c"]["min_required_files"]
                
                if def_files < needed:
                    print(f"      - process_c内のdef_*.datファイルが不足しています ({def_files}/{needed}必要)")
                
                if not results["required_folders"]["process_c"]["has_inp_csv"]:
                    print(f"      - process_c内にinp.csvファイルが見つかりません")
...
def main():
    ...
    if "process_c" in results["required_folders"]:
        ...
        if results["required_folders"]["process_c"]["exists"]:
            def_files_count = len(results["required_folders"]["process_c"]["found_files"]["def_*.dat"])
            has_inp_csv = results["required_folders"]["process_c"]["has_inp_csv"]
            
            # def_*.datファイルが十分かつinp.csvが存在するとき要件を満たす
            results["required_folders"]["process_c"]["files_requirement_met"] = (
                def_files_count >= args.min_files_c and has_inp_csv
            )
