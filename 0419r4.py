...
def find_process_folders(directory):
    ...
    try:
        ...
        if "process_c" in process_folders:
            ...
            pattern = os.path.join(directory, "process_c", "def_*.dat")
            matching_files = glob.glob(pattern)
            
            # ファイル名をリストに保存
            result["required_folders"]["process_c"]["file_list"] = [os.path.basename(f) for f in matching_files]
            result["required_folders"]["process_c"]["found_files"] = len(matching_files)
            
            # 必要な数のファイルが存在するかチェック
            min_required = result["required_folders"]["process_c"]["min_required_files"]
            if len(matching_files) >= min_required:
                result["required_folders"]["process_c"]["files_requirement_met"] = True
